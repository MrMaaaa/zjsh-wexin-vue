...
<div class="app">
  <div class="container">
    <div class="recharges flex-row">
      <section class="recharge-section" :class="{ active: activeIndex == index }" v-for="(item, index) in rechargeList" @touchend="selectRecharge(item, index)">
        <div class="content">
          <div class="money-recharge">充{{ item.RechargeMoney | formatMoney }}元</div>
          <div class="money-presented" v-if="!!item.ReturnMoney">赠送{{ item.ReturnMoney | formatMoney }}元</div>
        </div>
      </section>
    </div>
    ...
    <div class="recharge-amount flex-row">
      ...
      <div class="content">￥<span class="amount">{{ selectRechargeItem.RechargeMoney | formatMoney }}</span></div>
    </div>

    <div class="recharge-info flex-row">
      <div class="title">额外赠送&nbsp;&nbsp;{{ selectRechargeItem.ReturnMoney | formatMoney }}元</div>
      <div class="content">实际可得<span class="amount">￥{{ selectRechargeItem.TotalMoney | formatMoney }}</span></div>
    </div>
  ...
  <div class="pay-submit">
    <a class="btn-submit" @touchend="submitRecharge">￥{{ selectRechargeItem.RechargeMoney | formatMoney }}&nbsp;&nbsp;立即支付</a>
  </div>
</div>
</template>
...
export default {
  ...
  methods: {
    getRechargeList() {
      this.$loading.open(1);
      this.$store.dispatch('GetRechargeListEx')
        .then(res => {
          this.$loading.close();
    ...
    rechargeByWx(rechargeMoney, totalMoney, activityId, activity) {
      this.$store.dispatch('GetWxpaySignForWeb', {
        NeedPay: rechargeMoney,
        TotalMoney: totalMoney,
        ActivityId: activityId,
        Activity: activity,
