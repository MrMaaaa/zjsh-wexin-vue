...
<div>
  ...
  <transition name="fade">
    <div class="picker-wrapper" v-if="isDateTimePicker || isWeightPicker || isSelName || isPayWayPicker" @click="pickerClose"></div>
  </transition>
  ...
  <transition name="slide">
    <div class="picker-container" v-if="isPayWayPicker">
      <div class="picker-header flex-row">
  ...
  methods: {
    ...
    updateAddress() {
      ...
      if(this.addrData.sender.id && this.addrData.sender.id === this.addrData.receiver.id) {
        ...
        script.setAttribute('src', 'https://api.map.baidu.com/geocoder/v2/?' + window.decodeURIComponent(qs.stringify({
          ak: 'u8SIb8KCVSxsAsfEcd5bLv0LZuO4zDop',
          location: addr.Address1Lat + ',' + addr.Address1Lng,
          output: 'json',
    ...
    submitOrder() {
      if (this.selData.date != '' && this.selData.name != '' && this.selData.weight != '' && this.selData.way != '' && this.addrData.sender.show && this.addrData.receiver.show && this.addrData.sender.id != this.addrData.receiver.id) {
        this.$loading.open(2);
        this.$store.dispatch('CreateKdEOrder', {
        ...
        .then(res => {
          this.$loading.close();
          this.$alert('订单已提交，快递员将在2小时内上门取件');
          this.$router.push({
        ...
        else if (this.addrData.sender.id && this.addrData.sender.id === this.addrData.receiver.id) msg = '地址相同！请新增一个地址';
        this.$alert(msg);
      }
    },
  },
  computed: {
    ...mapState(['IsLogin', 'SelectedAddress', 'ALERT_MSG']),
  }
}
</script>
